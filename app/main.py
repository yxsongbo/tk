...
def build_session_snapshot(
    ...
    total_score = sum(float(item.get("score") or 0) for item in answers)

    answered_question_ids = {str(item.get("question_id")) for item in answers}
...
def get_session_state(
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, student_id, start_time, status, exam_filename FROM sessions WHERE id = %s",
        (session_id,),
    )
    ...
    session_exam = (session["exam_filename"] or "").strip()
    if not session_exam:
        session_exam = get_current_exam_name(conn) or ""
        cursor.execute(
            "UPDATE sessions SET exam_filename = %s WHERE id = %s",
            (session_exam, session_id),
        )
        conn.commit()
