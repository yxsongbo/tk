...
def _should_join_answer_fragments(answers):
    if len(answers) < 2:
        return False
...
def _expand_fill_blank_answers(answers):
...
def _extract_fill_correct_answer(question):
    ...
    for item in answers:
        if isinstance(item, dict):
            blank_answers = _expand_fill_blank_answers(_to_str_list(item.get("answers")))
            ...
            values = _expand_fill_blank_answers(_to_str_list(item))
            if values:
